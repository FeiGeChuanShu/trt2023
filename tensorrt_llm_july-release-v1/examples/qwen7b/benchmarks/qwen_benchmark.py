...
class QWENBenchmark(BaseBenchmark):

    def __init__(self, engine_dir, model_name, mode, batch_sizes, in_out_lens,
                 dtype, int8_kv_cache, use_weight_only, refit, num_beams, top_k, top_p, output_dir):
        ...
        if engine_dir is not None:
            ...
            self.use_gemm_plugin = self.config['plugin_config']['gemm_plugin']
            self.use_rmsnorm_plugin = self.config['plugin_config']['rmsnorm_plugin']
            ...
            self.max_output_len = self.config['builder_config'][
                'max_output_len']
            self.remove_input_padding = self.config['plugin_config']['remove_input_padding']
    ...
    def build(self):
        ...
        builder_config = builder.create_builder_config(
            ...
            int8=(self.quant_mode.has_act_and_weight_quant()
                  or self.quant_mode.has_int8_kv_cache()),
        ...
        self.rotary_dim = int((self.hidden_size // self.num_heads) * 1.0)
        # Initialize Module
        if self.model_name in ['qwen']:
            tensorrt_llm_model = tensorrt_llm.models.QWenLMHeadModel(
                num_layers=self.num_layers,
                num_heads=self.num_heads,
                hidden_size=self.hidden_size,
                vocab_size=self.vocab_size,
                ...
                tensor_parallel=self.world_size,
    ...
    def report(self, config, latency, percentile95, percentile99,
        ...
        if self.runtime_rank == 0:
            line = '[BENCHMARK] ' + (
                f'model_name: {self.model_name} precision: {self.dtype} \n'
                f'world_size: {self.world_size} num_heads: {self.num_heads} num_layers: {self.num_layers} hidden_size: {self.hidden_size} vocab_size: {self.vocab_size} \n'
