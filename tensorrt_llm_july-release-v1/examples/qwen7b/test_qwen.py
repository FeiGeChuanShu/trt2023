...
class TestLLaMA(unittest.TestCase):
    ...
    def _gen_tensorrt_llm_engine(self,
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            if use_plugin:
                ...
                #network.plugin_config.set_swiglu_plugin(dtype='float16')
            if fast_building:
    ...
    def test_qwen(self, use_refit, fast_building, context_fmha_flag,
        ...
        for i in range(qwen_config.num_hidden_layers):
            ...
            past_key_value_tensor = res_present_key_value.permute(1, 0, 3, 2, 4)
            key, value = past_key_value_tensor.chunk(2)

            key = key.reshape(batch_size, max_seq_len, qwen_config.num_attention_heads, head_size)
            value = value.reshape(batch_size, max_seq_len, qwen_config.num_attention_heads, head_size)

            ref_present_key, ref_present_value = hf_outputs.past_key_values[i]
            
            np.testing.assert_allclose(ref_present_key.cpu().numpy(),
                                       key[:, :input_len,:,  :].cpu().numpy(),
                                       atol=1e-1)

            compare_max_abs_error(ref_present_key, key[:, :input_len,:,  :],
                                  "ref_present_key.%d"%(i))

            np.testing.assert_allclose(ref_present_value.cpu().numpy(),
                                       value[:, :input_len,:,  :].cpu().numpy(),
                                       atol=1e-1)

            compare_max_abs_error(ref_present_value, value[:, :input_len,:,  :],
                                  "ref_present_value.%d"%(i))
