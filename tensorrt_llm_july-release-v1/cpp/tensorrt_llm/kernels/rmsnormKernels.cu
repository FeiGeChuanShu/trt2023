    ...
    int8_t* normed_output_quant)
{
    ...
    for (int i = tidx; i < n_elems; i += blockDim.x)
    {
        ...
        const float_packed_t val_f = cuda_cast<float_packed_t>(input[index]);
    ...
    if (with_per_token_scaling)
    {
        ...
        {
            const int index = bidx * n_elems + i;
            float_packed_t val_f = cuda_cast<float_packed_t>(input[index]);
    ...
    const size_t shmem_size, cudaStream_t stream)
{
    ...
    generalRmsNorm<T><<<grid, block, shmem_size, stream>>>(input, gamma, normed_output,
        eps, tokens, hidden_dim, scale_orig_quant_per_tensor, scale_orig_quant_per_token, normed_output_quant);
}
    ...
    int8_t* normed_output_quant, const dim3 grid, const dim3 block, const size_t shmem_size, cudaStream_t stream)
{
    ...
    dispatch_rmsnorm_type_square_method<T>(input, gamma, normed_output, eps, tokens, hidden_dim,
