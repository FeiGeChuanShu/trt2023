...
#undef LAUNCH_MASKED_SOFTMAX_
                                  ...
                                  const int size_per_head, const float *scale, int int8_mode)
        {
            ...
            if (int8_mode == 2)
            {
                ...
                reinterpret_cast<Int8_Packed_T *>(dst)[target_id] = cuda_cast<Int8_Packed_T>(cuda_cast<Float_Packed_T>(src[tid]) * scale_val);
            }
            else
            {
                                  ...
                                  const int size_per_head, const float *scale, int int8_mode)
        {
            ...
            const int target_id = batch_id * (head_num * seq_len * size_per_head) + seq_id * head_num * size_per_head + head_id * size_per_head + threadIdx.x;
...
#undef INSTANTIATE_TRANSPOSE_QKV
                                                         ...
                                                         const int *mask_offset)
        {
            ...
            for (int idx = threadIdx.x; idx < n; idx += blockDim.x)
            {
                ...
                const int tgt_id = tgt_batch_id * head_num * seq_len * size_per_head + tgt_head_id * seq_len * size_per_head + tgt_seq_id * size_per_head + tgt_hidden_id;
...
#undef INSTANTIATEADDQKVBIASREBUILDPADDING
                                                 ...
                                                 const int head_num, const int size_per_head, const int *mask_offset, const float *scale, const int int8_mode)
        {
            ...
            using Int8_Packed_T = typename packed_as<int8_t, num_elems<T>::value>::type;
            using Float_Packed_T = typename packed_as<float, num_elems<T>::value>::type;
            const Float_Packed_T scale_val = int8_mode == 2 ? cuda_cast<Float_Packed_T>(*scale) : cuda_cast<Float_Packed_T>(0.0f);
            ...
            for (int idx = threadIdx.x; idx < head_num * size_per_head; idx += blockDim.x)
            {
                ...
                if (int8_mode == 2)
                {
                    reinterpret_cast<Int8_Packed_T *>(dst)[dst_offset_base + idx] = cuda_cast<Int8_Packed_T>(cuda_cast<Float_Packed_T>(src_elem) * scale_val);
                }
                else
                {
...
#undef INSTANTIATE_TRANSPOSE_ATTENTION_OUT_REMOVE_PADDING
                                                           ...
                                                           const int head_num, const int size_per_head, const float *scale, const int int8_mode, const bool multi_query_mode)
        {
            ...
            for (int index = blockDim.x * blockIdx.x + threadIdx.x; index < token_num * n; index += gridDim.x * blockDim.x)
            {
                ...
                if (!multi_query_mode || qkv_id == 0) // QKV or Q when multi_query_mode
                {
                    ...
                    qkv_ptr[qkv_id][target_batch_id * target_batch_stride + head_id * target_head_stride + seq_id * target_seq_stride + size_id] = val;
                }
        ...
        template <typename T, bool ADD_BIAS>
        __global__ void add_fusedQKV_bias_transpose_kernel(T *q_buf, T *k_buf, T *v_buf, T *QKV, const T *__restrict qkv_bias,
                                                           const int *seq_lens, const int past_kv_len, const int *padding_offset, 
                                                           const int batch_size, const int seq_len, const int head_num,
                                                           const int size_per_head, const int rotary_embedding_dim, const int max_position_embeddings,
                                                           const bool neox_rotary_style, const bool multi_query_mode, const bool use_dynamic_ntk, 
                                                           const bool use_logn_attn)
        {
            ...
            // destination offset.
            const int dest_q_idx = batch_idx * size_per_head * seq_len * head_num + head_idx * size_per_head * seq_len + seq_idx * size_per_head + tidx * vec_size;

            const int kv_head_num = multi_query_mode ? 1 : head_num;
            const int dest_kv_idx = batch_idx * size_per_head * total_seq_len * kv_head_num + head_idx * size_per_head * total_seq_len + dst_kv_seq_idx * size_per_head + tidx * vec_size;
...
#pragma unroll
            ...
            else
            {
                ...
                if (do_rotary)
                {
                    ...
                    if (use_dynamic_ntk) {
                        const int context_len = past_kv_len + actual_seq_len;
                        rotary_emb_base = mmha::rotary_embedding_get_base(
                                context_len, max_position_embeddings, rotary_embedding_dim, rotary_emb_base);
                    }
                    mmha::apply_rotary_embedding(q, k , transpose_idx/ tidx_factor, rotary_embedding_dim, rotary_emb_base, dst_kv_seq_idx + past_kv_len);
            ...
            if (use_logn_attn) {
                ...
                float logn_scaling = mmha::logn_attn_get_scaling(dst_kv_seq_idx + past_kv_len + 1, max_position_embeddings);
...
#define FUSED_QKV_BIAS_ROTARY_TRANSPOSE_LAUNCH(T, ADD_BIAS)                                                                                                                                     \
    add_fusedQKV_bias_transpose_kernel<T, ADD_BIAS><<<grid, block, smem_size, stream>>>(q_buf, k_buf, v_buf, QKV,                                                                               \
                                                                                        qkv_bias, seq_lens, past_kv_len, padding_offset, batch_size, seq_len, head_num, size_per_head, rotary_embedding_dim, \
                                                                                        max_position_embeddings, neox_rotary_style, multi_query_mode, use_dynamic_ntk, use_logn_attn);
...
#define INSTANTIATE_ADDFUSEDQKVBIAS_TRANSPOSE(T)                                                                                                               \
    template void invokeAddFusedQKVBiasTranspose(T *q_buf, T *k_buf, T *v_buf, T *QKV, const T *qkv_bias,                                                      \
                                                 const int *seq_lens, const int past_kv_len, const int *padding_offset, const int batch_size,                  \
                                                 const int seq_len, const int token_num, const int head_num, const int size_per_head,                          \
                                                 const int rotary_embedding_dim, const int max_position_embeddings, const int neox_rotary_style,               \
                                                 const float *scale, const int int8_mode, const bool multi_query_mode, const bool use_dynamic_ntk,             \
                                                 const bool use_logn_attn, cudaStream_t stream)
