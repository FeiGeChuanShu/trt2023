...
inline __device__ float rotary_embedding_get_base(float seq_len, int max_position_embeddings, float rot_embed_dim, float base)
{
    if (seq_len < max_position_embeddings) {
        return base;
    }
...
inline __device__ float2 rotary_embedding_coefficient(const int zid, const int rot_embed_dim, const float base, const float t_step)
{
...
inline __device__ void apply_rotary_embedding(float& q, float& k, int zid, int rot_embed_dim, float base, int t_step)
{
    return;
}
...
inline __device__ void apply_rotary_embedding(uint32_t& q, uint32_t& k, int tid, int rot_embed_dim, float base, int t_step)
{
    if (2 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(half2& q, half2& k, int tid, int rot_embed_dim, float base, int t_step)
{
    return apply_rotary_embedding(
        *reinterpret_cast<uint32_t*>(&q), *reinterpret_cast<uint32_t*>(&k), tid, rot_embed_dim, base, t_step);
}
...
inline __device__ void apply_rotary_embedding(
    __nv_bfloat162& q, __nv_bfloat162& k, int tid, int rot_embed_dim, float base, int t_step)
{
    if (2 * tid >= rot_embed_dim)
    {
        return;
